...
3. **投影切分 + CNN 分类**：学号与分数均通过垂直投影自动切分字符，逐字符送入 CNN 分类，适配手写数字宽度不一、轻微粘连。
...
> 注：上表为历史评估值。学号已由「定长均分」改为「投影切分」，总分已加入「Σ小题」校验，两项当前准确率应高于表中所列，需用真实数据重新评估。
...
3. **学号识别（56.6%）对手写粘连、跨格仍敏感**：已从「定长均分」改为「投影切分」，但对手写潦草/跨格的情况仍有提升空间，可考虑引入 CTC 序列识别。
